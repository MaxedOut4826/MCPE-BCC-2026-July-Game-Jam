...
tag @e[tag=query.start_crawling] remove query.start_crawling
execute unless entity @s[tag=query.is_crawling,y=~0.7,dy=0] run tag @s add query.start_crawling

# query.stop_crawling: Player has stopped crawling
tag @e[tag=query.stop_crawling] remove query.stop_crawling
